...
- 2026-07-23: Retrieval integration design decision — the book corpus (~132K words across 27 chapters, ~70 world-bible files, editorial JSON, compiled omnibus) has no active semantic retrieval layer. The Meru FAISS index (3.2M chunks, `all-MiniLM-L6-v2`) is documented but absent from disk. Two operational embedding pipelines exist in the vault: COSMO-ALIGN (local MiniLM, 22 canonical posts) and Synchronocities (production `nv-embedqa-e5-v5` via Cloudflare Worker + Vectorize). The recommended path: a separate Cloudflare Worker with Vectorize binding using the Synchronocities-proven `nv-embedqa-e5-v5` model, ingesting the 27 chapter files + world bible + editorial metadata, exposed as a read-only retrieval endpoint. The webapp already has the Cloudflare Workers scaffold (Hono, D1) and `somatic_canticles_full_narrative_data.json` (2.2MB). This retrieval service is scoped outside the Ether-First controller, which prohibits network retrieval and model invocation per its ISA Constraints. The controller may consume retrieval results as pre-computed source-family inputs but must not invoke the retrieval service itself.
- 2026-07-23: Built the retrieval Worker at `workers/retrieval/` — a Hono-based Cloudflare Worker with Vectorize binding (`CORPUS_INDEX`), chunking library (500 words, 50-word overlap, YAML frontmatter stripping), embedding client (calls the Synchronocities Worker at `/test/eval-embed` for `nv-embedqa-e5-v5`), ingestion pipeline (batch upsert into Vectorize), and search endpoint (embed query → Vectorize query → ranked results). TypeScript compiles clean. The Worker is not yet deployed — it requires `wrangler vectorize create somatic-canticles-corpus --dimensions=1024 --metric=cosine` and a corpus file payload before the first ingestion.
